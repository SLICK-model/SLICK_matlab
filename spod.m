function [L,P,f,Lc,A,Q_hat] = spod(X,varargin)
...
switch xfun
    ...
    case false
        if blk_mean
            mean_name   = 'blockwise mean';
        elseif isfield(opts,'mean')&&(~blk_mean)
            x_mean      = opts.mean(:);
            mean_name   = 'user specified';      
        else
            x_mean      = mean(X,1);
...
if ~opts.savefft
    ...
    Q_hat   = reshape(Q_hat,[nFreq dim(2:end) nBlks]);
end
